    ...
    namespace View {
        ...
        QVariant EntityPropertyModel::data(const QModelIndex& index, const int role) const {
            ...
            if (role == Qt::ToolTipRole) {
                if (index.column() == 0) {
                    if (row.isProtected() != PropertyProtection::NotProtected) {
                        return QVariant("Property is protected from changes in linked groups");
                    }
                    return QVariant("Property shares its value with linked groups");
                } else {
